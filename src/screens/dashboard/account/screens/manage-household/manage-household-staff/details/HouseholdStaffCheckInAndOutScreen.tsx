...
import { getHouseholdStaffCheckInOut } from '@src/api/household.api';
...
const HouseholdStaffCheckInAndOutScreen = (): React.JSX.Element => {
  const { selectedHouseholdStaff } = useAppStateStore();
  const id = selectedHouseholdStaff?.id || 0;

  const queryKey = [
    queryKeys.GET_HOUSEHOLDS,
    'getHouseholdStaffCheckInOut',
    id,
  ];
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } =
    useInfiniteQuery({
      ...
      queryFn: async ({ pageParam }) => {
        const response = await getHouseholdStaffCheckInOut({
...
export default HouseholdStaffCheckInAndOutScreen;
