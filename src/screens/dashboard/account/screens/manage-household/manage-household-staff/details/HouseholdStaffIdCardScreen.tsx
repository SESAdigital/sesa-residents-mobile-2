...
import { getHouseholdStaffIdCard } from '@src/api/household.api';
...
const HouseholdStaffIdCardScreen = (): React.JSX.Element => {
  const { selectedHouseholdStaff } = useAppStateStore();
  const id = selectedHouseholdStaff?.id;
  const queryClient = useQueryClient();
  const viewShotRef = useRef<ViewShot>(null);
  const queryKey = [queryKeys.GET_HOUSEHOLDS, 'getHouseholdStaffIdCard', id];
  const { data, isLoading } = useQuery({
    queryKey,
    queryFn: async () => {
      const response = await getHouseholdStaffIdCard(id!);
      if (response.ok) {
    ...
    try {
      if (viewShotRef?.current && viewShotRef?.current?.capture) {
        ...
        if (image) {
          await Share.open({
            url: image,
            message: 'SESA Household Staff ID Card',
          });
        }
      }
    } catch (error) {
...
export default HouseholdStaffIdCardScreen;
