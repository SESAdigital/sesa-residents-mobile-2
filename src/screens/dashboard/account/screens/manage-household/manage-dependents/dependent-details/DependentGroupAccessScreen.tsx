...
import { getPropertyDependentsGroupAccess } from '@src/api/household.api';
...
const DependentGroupAccessScreen = (): React.JSX.Element => {
  ...
  const queryKey = [
    ...
    'getPropertyDependentsGroupAccess',
    id,
    propId,
  ];
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } =
    useInfiniteQuery({
      ...
      queryFn: async ({ pageParam }) => {
        const response = await getPropertyDependentsGroupAccess({
  ...
  return (
    <FlatList
      ...
      renderItem={({ item }) => <GenericGroupAccessRow value={item} />}
...
export default DependentGroupAccessScreen;
