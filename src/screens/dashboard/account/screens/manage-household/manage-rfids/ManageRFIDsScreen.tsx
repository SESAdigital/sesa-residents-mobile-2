...
import { getHouseholdRFIDs } from '@src/api/household.api';
...
import EmptyRFIDComponent from './components/EmptyRFIDComponent';
import RFIDRow, { RFIDRowLoader } from './components/RFIDRow';

type Props = AppScreenProps<'MANAGE_RFIDS_SCREEN'>;

const pageSize = DEFAULT_API_DATA_SIZE;

const ManageRFIDsScreen = ({ route }: Props): React.JSX.Element => {
  const { id, name } = route?.params;

  const queryKey = [queryKeys.GET_HOUSEHOLDS, 'getHouseholdRFIDs', id];
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } =
    useInfiniteQuery({
      ...
      queryFn: async ({ pageParam }) => {
        const response = await getHouseholdRFIDs({
  ...
  return (
    <AppScreen showDownInset>
      <AppScreenHeader>
        <AppText style={styles.headerTitle}>Manage RFIDs</AppText>
        {isLoading ? (
          <AppSkeletonLoader
      ...
      {formattedData?.length > 0 && (
        <AppText style={styles.instructions}>Tap RFID for view details</AppText>
      )}
      ...
      <FlatList
        ...
        ListEmptyComponent={
          isLoading ? (
            <DuplicateLoader loader={<RFIDRowLoader />} />
          ) : (
            <EmptyRFIDComponent />
          )
        }
        renderItem={({ item }) => (
          <RFIDRow
            onPress={() =>
              navigation.navigate(routes.RFID_HISTORY_SCREEN, item)
            }
            data={item}
          />
        )}
...
export default ManageRFIDsScreen;
